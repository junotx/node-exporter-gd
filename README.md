...
To install or upgrade the chart with the release name `prometheus-node-exporter` into the namespace `kube-monitoring-sytem`:

```bash
helm upgrade --install prometheus-node-exporter ./pkg/prometheus-node-exporter-4.37.2.tgz -f ./pkg/custom-values.yaml --namespace kube-monitoring-system --create-namespace
```

To enable `calico-exporter` and `process-exporter`, you can set the following values:
```bash
helm upgrade --install prometheus-node-exporter ./pkg/prometheus-node-exporter-4.37.2.tgz -f ./pkg/custom-values.yaml --set CalicoExporter.enabled=true --set ProcessExporter.enabled=true --namespace kube-monitoring-system --create-namespace
```
